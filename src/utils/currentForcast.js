...
const currentForecast = (lat, lon, callback) => {
    const urlCurrent = `https://api.climacell.co/v3/weather/realtime?unit_system=us&apikey=Mj8MQkOElh4cIm69zHBP5CjOyvYBP3cu&lat=${lat}&lon=${lon}&fields=wind_speed,temp,wind_direction,feels_like,humidity,precipitation,precipitation_type`
    request({ url: urlCurrent, json: true }, (error, response, { errorCode, message: msgError, temp, wind_speed, wind_direction, feels_like, humidity, precipitation, precipitation_type } = {}) => {
        if (error) {
