...
weatherForm.addEventListener("submit", (e) => {
  ...
  fetch(`/weather?address=${location}`).then((response) => {
    response.json().then((data) => {
      ...
      if (data.errorMsg) {
        weatherHeaderP.textContent = "ERROR";
...
function displayCurrentWeather(geocode, currentForecast) {
  ...
  weatherCode.textContent = `Conditions: ${capitalizeString(normalizeString(
  ...
  if (currentForecast.precipitationType != null) {
    currentPrecipitation.textContent = `Precipitation: ${capitalizeString(
      currentForecast.precipWord
    )} ${capitalizeString(currentForecast.precipitationType)} (${
...
function createForecastList({ hourWeather: hours }) {
  ...
  const temperatureIcon = createWeatherIcon(
    "/img/temperature.svg",
    "25",
    "small-temp-icon",
    "temp gauge"
  );
  const raidDropIcon = createWeatherIcon(
    "/img/drop.svg",
    "15",
    "small-precip-icon",
    "rain drop"
